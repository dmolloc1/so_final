...
const ClienteForm: React.FC<ClienteFormProps> = ({
  ...
  const normalizeDate = (date: string | null | undefined): string => {
    if (!date) return "";
    // Acepta: 2025-11-19 o 2025-11-19T00:00:00Z
  ...
  const validate = (): boolean => {
    ...
    if (!formData.cli_fecha_nac) {
      newErrors.cli_fecha_nac = 'La fecha de nacimiento es obligatoria';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.cli_dni) setErrors(prev => ({ ...prev, cli_dni: error.cli_dni[0] }));
      if (error.cli_email) setErrors(prev => ({ ...prev, cli_email: error.cli_email[0] }));
      if (error.cli_telefono) setErrors(prev => ({ ...prev, cli_telefono: error.cli_telefono[0] }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label
              ...
              className="block text-sms mb-2 font-medium text-gray-700"
            >
