...
import RecetaForm from "./RecipeForm";
...
export default function RecetasPage({
  ...
  const loadRecipes = async () => {
    try {
      ...
      const filters: any = {
        ...
        recTipoLente: tipoLenteFilter || undefined,
        optometra: optometraFilter || undefined,
      };

      if (clientFilter) {
        filters.cliente = clientFilter;
      } else if (searchTerm) {
      ...
      if (fechaInicio) {
        data = data.filter(
          (r) =>
            r.recFecha &&
            new Date(r.recFecha) >= new Date(fechaInicio)
        );
      }
      ...
      if (fechaFin) {
        data = data.filter(
          (r) =>
            r.recFecha &&
            new Date(r.recFecha) <= new Date(fechaFin)
        );
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: Recipe) =>
        row.recFecha
          ? new Date(row.recFecha).toLocaleDateString()
          : "—",
    },
    {
    ...
    },
    {
      ...
      render: (row: Recipe) => (
        <div className="flex justify-center gap-2">
          ...
          <button
            onClick={async () => {
              if (row.recCod && confirm("¿Deseas eliminar esta receta?")) {
                await recipeService.delete(row.recCod);
                loadRecipes();
              }
            }}
        ...
        onSubmit={async (data) => {
          try {
            if (editingRecipe?.recCod) {
              await recipeService.update(editingRecipe.recCod, data);
              notifySuccess("Receta actualizada exitosamente.");
            } else {
              const payload: Omit<Recipe, "recCod"> = {
                ...data,
                recFecha: data.recFecha ?? new Date().toISOString(),
              };
