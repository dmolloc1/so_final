...
const BranchInventory: React.FC<BranchInventoryProps> = ({ user }) => {
  ...
  const columns: Column<ProductInBranch>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${row.origen === 'GLOBAL'
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <p
            className={`text-lg font-bold ${row.bajo_stock ? 'text-red-600' : 'text-green-600'
              }`}
          >
